...
export function Game({ user, onSignOut }) {
    ...
    // attemptCount is 0-4 for the 5 attempts in a round
    ...
    // **FIXED:** Calculation for display (1-5 instead of 0-4)
    const displayAttemptCount = (attemptCount % 5) + 1;
    ...
    const startNewRound = useCallback(async (currentDifficulty) => {
        ...
        try {
            ...
            // **FIXED:** Increment attempt count and round count
            // Note: attemptCount is 0-indexed (0 to 4). It is incremented AFTER fetch and will be used 
            // by handleClassification as the pre-incremented state value if necessary, but 
            // the new value will only be visible in the next render cycle.
            // For round-cycling, we check the *new* value after incrementing it.
            setAttemptCount(prevCount => {
                const newCount = (prevCount + 1) % 5;
                if (newCount === 0) {
                    setTotalRoundsPlayed(prevRounds => prevRounds + 1);
                }
                return newCount;
            });
        ...
        const fetchUserDataAndSession = async () => {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    setAttemptCount(0); // Reset attempt count on login
                }
            } catch (error) {
                ...
                try {
                    ...
                    if (sessionData.userId === user.uid) {
                        ...
                        setStats(prevStats => ({
                            ...
                            // totalCorrect/Incorrect are persistent, so we stick to what DB/Initial Load provided
                        }));
                        setAttemptCount(sessionData.attemptCount);
                        setTotalRoundsPlayed(sessionData.totalRoundsPlayed);
                        setIsSessionActive(true); // Trigger the session modal
                    } else {
    ...
    const resumeSession = () => {
        ...
        // Restore session-specific states
        ...
        setIsSessionActive(false); // Close the modal
    };
    ...
    const startNewGame = () => {
        ...
        setStats(prev => ({ 
            ...
            currentScore: 0, 
            currentStreak: 0,
            highestScore: prev.highestScore, // Keep permanent highest score
            highestStreak: prev.highestStreak, // Keep permanent highest streak
        }));
    ...
    const handleClassification = (choice) => {
        ...
        setStats(prevStats => {
            ...
            // Since setTotalRoundsPlayed happens asynchronously in startNewRound,
            // we calculate the roundsPlayed to send to the DB based on the new attemptCount
            const isRoundEnd = displayAttemptCount === 5;
            if (isRoundEnd) currentRounds++;


            // Update DB with the NEW state values
    ...
    const handleSignOut = useCallback(async () => {
        sessionStorage.removeItem(GAME_SESSION_KEY);
        // Persist final highest scores/streaks, but zero out current session scores/streaks
        const finalStats = { 
            ...stats, 
            currentScore: 0, 
            currentStreak: 0 
            ...
            <div className="game-container fullscreen-layout">
                <div className="custom-modal-overlay">
                    <div className="custom-modal-content session-prompt">
                        ...
                        {/* **FIXED:** Changed displayAttemptCount to use the 1-5 value */}
            ...
            <header className="game-header ribbon-layout" style={{ position: 'relative' }}>
                ...
                <div className="header-right ribbon-right" style={{ position: 'relative' }} ref={dropdownRef}>
                    ...
                    <div style={dropdownStyles}>
                        ...
                        <button
                            style={{ ...dropdownButtonStyles, ...(editProfileOpen ? { background: '#444' } : {}) }}
